...
toll-booth turns any HTTP API into a vending machine. One middleware. Multiple payment protocols - Lightning, Cashu ecash, x402 stablecoins and IETF Payment. NWC is one operator-side Lightning backend, not a client payment rail. The API operator picks what to accept. The client picks how to pay. Neither needs permission from the other.
...
- **Payment pluralism beats vendor lock-in.** Five Lightning backends including NWC, plus Cashu-only mode — use what you already run.
