...
**`url` tag:** The base URL of the service. Clients append capability-specific paths as needed. MUST be an HTTPS URL. For single-endpoint services, this is the full endpoint URL.
...
Implementations SHOULD verify that the pubkey's [NIP-05](05.md) identifier domain matches the `url` tag domain. This provides an anti-spoofing signal: a service claiming to be at `https://example.com` should have a NIP-05 on `example.com`.
